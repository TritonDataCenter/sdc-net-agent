...
function NetAgent(options) {
    ...
    this.userAgent = 'net-agent/' + this.version +
...
NetAgent.prototype._updateVmNics = function (options, callback) {
    ...
    // 1. NIC was removed from the VM or VM provision has failed
    ...
    // Set an origin header with all NIC update requests by default
    var defaultOptions = { headers: { origin: 'net-agent'} };

    function _addNic(vmNic, cb) {
        var newNic = _createNicPayload(options.vm, vmNic);

        napi.createNic(newNic.mac, newNic, defaultOptions, function (err) {
            if (err) {
    ...
    function _updateNic(vmNic, napiNic, cb) {
        ...
        napi.updateNic(newNic.mac, newNic, defaultOptions, function (err) {
            if (err) {
    ...
    function _deleteNic(vmNic, cb) {
        napi.deleteNic(vmNic.mac, defaultOptions, function (err) {
            if (err) {
                if (err.name === 'ResourceNotFoundError') {
