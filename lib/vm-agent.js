...
function VmAgent(options) {
    ...
    this.queues = {};
    // Keeps a helpful cache of updates that get sent identified by reqId
    this.updateCache = {};
}
...
VmAgent.prototype.start = function () {
    ...
    this.updateAgent.on('updated', function (uuid, reqId) {
        var cacheObj = updateCache[reqId];

        log.info(cacheObj, 'Update for %s delivered', uuid);
        // Not needed anymore
        delete updateCache[reqId];
    });

    this.updateAgent.on('error', function (err, uuid, reqId) {
        var cacheObj = updateCache[reqId];
        cacheObj.err = err;

        log.info(cacheObj, 'Update for %s failed', uuid);
        // Not needed anymore
        delete updateCache[reqId];
    });
};
...
VmAgent.prototype._queueSample = function (options) {
    var self = this;
    var uuid, apiPath, query, payload, cacheObj;

    if (options.uuid) {
        uuid = options.uuid;
        apiPath = format(VM_PATH, options.uuid);
        query = {};
        payload = options.sample[uuid];
        cacheObj = { uuid: options.uuid, state: options.sample[uuid].state };
    } else {
        uuid = self.uuid;
        apiPath = '/vms';
        query = { server_uuid: uuid };
        payload = { vms: options.sample };
        cacheObj = { server_uuid: uuid };
    }
    ...
    if (this.updateAgent) {
        ...
        var reqId = this.updateAgent.queueUpdate(uuid, message);
        cacheObj.req_id = reqId;
        this.updateCache[reqId] = cacheObj;
    }
};
