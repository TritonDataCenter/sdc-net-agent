...
EventWatcher.prototype.startStateWatcher = function () {
    var self = this;
    var log = this.log;
    var vmStates = this.vmStates;

    function onLookup(err, vmobjs) {
        if (err) {
            log.error(err, 'Unable update run stateWatcher lookup');
        ...
        vmobjs.forEach(function (vm) {
            ...
            if (vmStates[vm.uuid] !== vm.state) {
                self.emit('state', vm.uuid, vm.state);
            }
            vmStates[vm.uuid] = vm.state;
        });
    ...
    var lookup = listVMs.bind(null,
        {}, ['uuid', 'state', 'do_not_inventory'], onLookup);
